...
**內側鍵 (Inner Key)**:
兩食指內側的鍵(左 T/G/B、右 Y/H/N)。用途:同手「疊放」整組收斂成的**單一強調鍵**,依錨點那顆的列取上/中/下。單顆音符與跨手同拍不使用。

**疊放 (Stack)**:
同一手、彼此相距 **< 1/8 beat** 的多顆音符;在 Beat Saber 意義上是「一次揮砍的強調」。以錨點制成群(只跟該組第一顆比),整組收斂成一顆內側鍵音符(見「內側鍵」)。

**連打 (Burst)**:
同一手、相距 **≥ 1/8 beat** 的連續音符;是原譜本來就有的節奏,各自保留正常映射,不特殊處理(與「疊放」互斥)。
...
**道 (Lane)**:
高速公路上音符飛來的水平位置,共 **10 道**照實體鍵盤由左到右排:左手小指/無名/中指/食指、中央左內側(TGB)/右內側(YHN)、右手食指/中指/無名/小指。**列(上/中/下排)以高度(Y)呈現**——上排飛高、下排飛低。故一顆音符的 (道, 列) = 該鍵在鍵盤上的 (左右, 上下)。內側鍵雖屬食指,仍佔獨立道(見 docs/adr/0006)。
