...
- `?holdtest`:含三顆長按(issue 08、issue 27)。A=`KeyF` 頭 0.6/尾 2.0(有尾部判定)、B=`KeyJ` 頭 2.5/尾 3.6(有尾部判定)、C=`KeyD` 頭 0.6/尾 2.0(**無尾部判定**,與 A 同時但不同鍵,方便對照)。驗長按判定/回饋:自動打按下頭部後持續發光(目標格常亮金/綠)+ 長條沿走廊拖到判定平面(C 的長條應呈半透明,與 A/B 不透明可視覺區分);A/B 撐過破壞點(尾−goodSec)放開或按到尾→鎖定(金脈衝+高音完成音),提早放開→破(MISS 紅閃+斷 combo);C 不論何時放開都不罰(維持頭部判定、不斷 combo),尾部鎖定時只有金/綠脈衝、**不響完成音**。
