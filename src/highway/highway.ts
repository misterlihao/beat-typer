...
// 幾何:X=道(10 道照鍵盤排)、Y=列高度(上高下低)、Z=飛行。見 docs/adr/0002、0006、0014 與 CONTEXT.md。
...
import {
  highwayReducer,
  INITIAL_HIGHWAY_STATE,
  type HighwayEffect,
  type HighwayEvent,
  type HighwayState,
} from './highwayState.ts';
import {
  buildControls,
  buildCountdown,
  buildFeedback,
...
export function startHighway(
  ...
  container.appendChild(buildInfoCard({ songName: deps.songName, difficultyLabel: deps.difficultyLabel, coverUrl: deps.coverUrl }));
  ...
  // 顯示狀態純粹跟隨 hwState.tailReached(見 dispatch),此函式只管 DOM。
  ...
  // ── 遊戲流程狀態機(見 highwayState.ts):hwState 為唯一真相,dispatch 是進入點。 ──
  // runEffect 把 reducer 決定的 effect 接上真正的音訊 / DOM / 計時器副作用;呼叫順序即 reducer 回傳順序。
  let hwState: HighwayState = INITIAL_HIGHWAY_STATE;
  const dispatch = (event: HighwayEvent): void => {
    const { state: next, effects } = highwayReducer(hwState, event);
    hwState = next;
    setTailHint(hwState.tailReached);
  ...
  const loop = (ts: number) => {
    ...
    if (judger) {
      ...
      if (judger.allResolved) dispatch({ type: 'allNotesResolved' }); // reducer 內建冪等,只在首次真正轉態
    }
    showProgress();
    ...
    if (autoPlay && hwState.phase === 'playing') autoDrive(now); // ?auto:音訊時鐘驅動的合成按鍵
    positionNotes(now);
  ...
  positionNotes(-Infinity); // 進場靜態畫面(格線);隨後 dispatch('restart') 自動倒數開跑
  renderer.render(scene, camera);
  ...
  const REVEAL_PX = 140;
  let barPinned = true; // 初值;進場 dispatch('restart') 進倒數即轉為自動顯隱
  ...
  const cursorAutoHides = () => hwState.phase === 'countdown' || hwState.phase === 'playing';
  ...
  // runEffect:把 highwayReducer 決定的 effect 接上真正的音訊 / DOM / 計時器副作用。
  // 除 'launch' 的非同步分支外,皆同步執行完畢——effect 執行順序即 reducer 回傳的陣列順序。
  const runEffect = (effect: HighwayEffect): void => {
    switch (effect.type) {
      case 'resetForNewRun':
      ...
      case 'cancelCountdown':
        // 倒數被打斷 → 回暫停覆蓋層(launchKind 不變:之後「繼續」重跑倒數→同一種續播)。
        clearCountdown();
        countdownEl.style.display = 'none';
        revealCursor();
        showOverlay('paused');
      ...
      case 'launch':
        // freshLaunch=從 0(首玩/重玩),resumeLaunch=從凍結位置(暫停續玩)。
        if (effect.kind === 'fresh') {
          void (async () => {
            await player.play(0);
            startLoop();
      ...
      case 'finish':
        // 收尾結算(自然播畢 / 尾段提早結束共用):補判、停迴圈、寫入成績、顯示結算覆蓋層。
        // 用 player.positionSec 當末端時間——自然播畢時它已等於 duration,尾段直接結束前先
        // pause 凍結於當下位置(player.pause() 對已停止的 player 是安全 no-op,見 AudioPlayer)。
        player.pause();
        if (judger) {
      ...
      default: {
        // 窮盡性錨點:runEffect 回傳 void,少處理一種 HighwayEffect 不會被 TS 抓到(不像
        // highwayReducer 因回傳 HighwayTransition 而天生窮盡)。這裡用 never 賦值把它釘住——
        // 日後 HighwayEffect 加新變體卻忘了在此加對應 case,編譯期就會報錯。
        const exhaustive: never = effect;
        throw new Error(`未處理的 highway effect: ${JSON.stringify(exhaustive)}`);
      }
    }
  };

  resumeBtn.addEventListener('click', () => dispatch({ type: 'resumeClicked' }));
  ...
  player.onEnded = () => dispatch({ type: 'naturalEnd' });

  // Space / Escape:遊玩→暫停、暫停→繼續(倒數)、倒數→打斷回暫停(非遊戲鍵;preventDefault 防捲動)。
  // 尾段(所有音符判定完)時兩鍵改為「直接結束」,由 reducer 依 tailReached 決定——見 highwayState.ts。
  const onControlKey = (e: KeyboardEvent) => {
    if (e.repeat || (e.code !== 'Space' && e.code !== 'Escape')) return;
    if (hwState.phase !== 'playing' && hwState.phase !== 'paused' && hwState.phase !== 'countdown') return;
    e.preventDefault();
    dispatch({ type: 'controlKey' });
  };
  ...
  const onVisibility = () => {
    ...
    if (hwState.phase !== 'playing' && hwState.phase !== 'countdown') return;
    dispatch({ type: 'tabHidden' });
  };
